 ...
 *
 * Commit-reveal coinflip with improved fairness guarantees.
 *
 * FAIRNESS IMPROVEMENTS OVER v2-final:
 ...
 *
 * WHY NOT MULTI-BLOCK HASHING:
 *   ErgoScript does NOT support looking up historical block hashes by height.
 *   Only CONTEXT.preHeader (current block) is available. The v3 draft used
 *   "CONTEXT HEIGHT rngBlockHeight" which is invalid ErgoScript syntax.
 *   Production-grade fairness requires an oracle or a two-phase commit where
 *   the house pre-commits to a block hash, but this adds complexity.
 *
 * SECURITY MODEL (PoC+):
