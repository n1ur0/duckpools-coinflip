...
import './MosaikCoinFlipGame.css';

// ─── Mosaik WASM Integration ──────────────────────────────────────────────

// Import sigma-rust WASM module
let sigmaRust: any = null;
let sigmaRustReady = false;

async function initSigmaRust() {
  if (sigmaRustReady) return;
  
  try {
    // Dynamic import of sigma-rust WASM
    const module = await import('../../../wasm/sigma-rust');
    sigmaRust = module;
    sigmaRustReady = true;
    console.log('[Mosaik] sigma-rust WASM module loaded successfully');
  } catch (error) {
    console.error('[Mosaik] Failed to load sigma-rust WASM:', error);
    sigmaRustReady = false;
  }
}
...
interface MosaikCoinFlipGameProps {
  className?: string;
}

const MosaikCoinFlipGame: React.FC<MosaikCoinFlipGameProps> = ({ className = '' }) => {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (isOnChainEnabled()) {
        ...
        // Use sigma-rust WASM for enhanced wallet operations if available
        let playerPubKey = extractPubKeyFromUtxo(utxos[0]);
        if (!playerPubKey && sigmaRustReady) {
          // Fallback: use sigma-rust to extract public key
          try {
            playerPubKey = await sigmaRust.extractPublicKey(utxos[0]);
          } catch (err) {
            console.warn('[Mosaik] Failed to extract public key with sigma-rust:', err);
          }
        }
        ...
        console.log(`[Mosaik] On-chain bet placed: txId=${txId}, timeout=${timeoutHeight}`);
      } else {
        // ── OFF-CHAIN FALLBACK ─────────────────────────────────
        ...
        console.warn(
          '[Mosaik] OFF-CHAIN MODE — contract not compiled yet (MAT-344). ' +
  ...
  if (!isConnected) {
    return (
      <div className={`mosaik-coinflip-game-container ${className}`}>
        <div className="mosaik-coinflip-connect-prompt">
          <p>Connect your wallet to start flipping</p>
          <Button 
            className="mosaik-coinflip-connect-btn" 
            onClick={connect}
            variant="primary"
            size="lg"
          >
  ...
  return (
    <div className={`mosaik-coinflip-game-container ${className}`}>
      <h2 className="mosaik-coinflip-title">Mosaik Coin Flip</h2>

      {!isOnChainEnabled() && (
        <div className="mosaik-coinflip-offchain-banner">
          ⚠️ Off-chain mode — contract not yet deployed (MAT-344)
        </div>
      )}

      <div className="mosaik-coinflip-game-layout">
        {/* ── Main Game Area ───────────────────────────────────────── */}
        <div className="mosaik-coinflip-main-area">
          {/* ── Coin Visual Area ───────────────────────────────────── */}
          <div className="mosaik-coinflip-visual-area">
            <div className="mosaik-coinflip-board">
              <CoinFlip 
              ...
              />
              {!isFlipping && result && (
                <div className="mosaik-coinflip-result-section">
                  {/* Enhanced result display with icon */}
                  <div className={`mosaik-coinflip-result-wrapper ${result}`}>
                    <div className={`mosaik-coinflip-result-icon ${result}`}>
                      {result === 'heads' ? '◉' : '◎'}
                    </div>
                    <div className={`mosaik-coinflip-result-text ${result}`}>
                  ...
                  {winOutcome && (
                    <div className={`mosaik-coinflip-outcome ${winOutcome}`}>
                      <div className="mosaik-coinflip-outcome-icon">
                        {winOutcome === 'win' ? '✦' : '✕'}
                      </div>
                      <div className="mosaik-coinflip-outcome-text">
                        {winOutcome === 'win' ? 'YOU WIN!' : 'YOU LOSE'}
                      </div>
                      {betPlaced && (
                        <div className="mosaik-coinflip-bet-amount">
          ...
          {/* ── Choice Buttons ───────────────────────────────────── */}
          <div className="mosaik-coinflip-choice-section">
            <span className="mosaik-coinflip-choice-label">Pick Your Side</span>
            <div className="mosaik-coinflip-choice-buttons">
              <Button
                className={`mosaik-coinflip-choice-btn mosaik-coinflip-choice-btn--heads${
                  choice === 0 ? ' mosaik-coinflip-choice-btn--selected' : ''
                }`}
              ...
              <Button
                className={`mosaik-coinflip-choice-btn mosaik-coinflip-choice-btn--tails${
                  choice === 1 ? ' mosaik-coinflip-choice-btn--selected' : ''
                }`}
        ...
        <div className="mosaik-coinflip-bet-sidebar">
          {/* ── Amount Input ───────────────────────────────────────── */}
          <div 
            className="mosaik-coinflip-amount-section mosaik-coinflip-touch-area"
            onTouchStart={handleTouchStart}
            onTouchEnd={handleTouchEnd}
          >
            <label className="mosaik-coinflip-amount-label">
              Bet Amount
              <span className="mosaik-coinflip-amount-hint">
                Swipe ↑↓ to adjust
              </span>
            </label>
            <Input
              className="mosaik-coinflip-amount-input"
              type="text"
              inputMode="decimal"
              placeholder="0.0"
              value={amount}
            ...
            />
            <div className="mosaik-coinflip-quick-picks">
              {QUICK_PICK_VALUES.map((val) => (
                <Button
                  key={val}
                  className="mosaik-coinflip-quick-pick"
                  variant="ghost"
                  size="sm"
            ...
            {isValidAmount && (
              <div className="mosaik-coinflip-payout-preview">
          ...
          {/* ── Submit ─────────────────────────────────────────────── */}
          <Button
            className={`mosaik-coinflip-submit-btn${isSubmitting ? ' mosaik-coinflip-submit-btn--loading' : ''}`}
            variant="primary"
            size="lg"
          ...
          {/* ── Info ─────────────────────────────────────────────── */}
          <div className="mosaik-coinflip-info">
            <span className="mosaik-coinflip-info-item">
      ...
      {/* ── Error ────────────────────────────────────────────────── */}
      {error && <div className="mosaik-coinflip-error">{error}</div>}

      {/* ── Result Actions (for when on-chain reveal completes) ─── */}
      {!isFlipping && result && winOutcome && (
        <div className="mosaik-coinflip-result-actions">
          <button
            className="mosaik-coinflip-flip-again-btn"
            onClick={() => {
              setResult(null);
              setWinOutcome(null);
              setBetPlaced(null);
      ...
      {betPlaced && !result && (
        <div className="mosaik-coinflip-pending">
          <div className="mosaik-coinflip-pending-title">
            <span className="mosaik-coinflip-pending-spinner" />
            Bet Placed — Awaiting On-Chain Reveal
          </div>
          <div className="mosaik-coinflip-pending-row">
            <span className="mosaik-coinflip-pending-row-label">Bet ID</span>
            <span className="mosaik-coinflip-pending-row-value">{betPlaced.betId.slice(0, 16)}...</span>
          </div>
          <div className="mosaik-coinflip-pending-row">
            <span className="mosaik-coinflip-pending-row-label">Amount</span>
            <span className="mosaik-coinflip-pending-row-value">{betPlaced.amount} ERG</span>
          </div>
          <div className="mosaik-coinflip-pending-row">
            <span className="mosaik-coinflip-pending-row-label">Choice</span>
            <span className="mosaik-coinflip-pending-row-value">{betPlaced.choiceLabel}</span>
          </div>
          <div className="mosaik-coinflip-pending-row">
            <span className="mosaik-coinflip-pending-row-label">Commitment</span>
            <span className="mosaik-coinflip-pending-row-value">{betPlaced.commitment.slice(0, 16)}...</span>
          </div>
          {betPlaced.txId && (
            <div className="mosaik-coinflip-pending-row">
              <span className="mosaik-coinflip-pending-row-label">Transaction</span>
              <span className="mosaik-coinflip-pending-row-value">
                <a
                  href={`https://explorer.ergoplatform.com/en/transactions/${betPlaced.txId}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="mosaik-coinflip-tx-link"
                >
          ...
          <div className="mosaik-coinflip-pending-note">
            {isOnChainEnabled()
...
export default MosaikCoinFlipGame;