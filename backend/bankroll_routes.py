...
def _get_ergo_tree_hash() -> str:
    """Compute SHA256 hash of the coinflip contract ergoTree bytes."""
    global _ergo_tree_hash
    if _ergo_tree_hash is not None:
        return _ergo_tree_hash
    try:
        from game_routes import COINFLIP_ERGO_TREE
        ergo_tree_bytes = bytes.fromhex(COINFLIP_ERGO_TREE)
        _ergo_tree_hash = hashlib.sha256(ergo_tree_bytes).hexdigest()
        logger.info("Computed coinflip ergoTree hash: %s...", _ergo_tree_hash[:16])
        return _ergo_tree_hash
    except Exception as e:
        logger.error("Failed to compute ergoTree hash: %s", e)
        return ""
...
# ═══════════════════════════════════════════════════════════════════════
# Bankroll Deposit/Withdrawal Ledger (Phase 3)
# ═══════════════════════════════════════════════════════════════════════

from services.bankroll_ledger import (
    get_balance as get_ledger_balance,
    get_entries as get_ledger_entries,
    init_db as init_ledger_db,
    record_entry as record_ledger_entry,
)
...
class DepositRequest(BaseModel):
    """Record a deposit to the house bankroll."""
    tx_id: str = Field(..., min_length=64, max_length=64, description="On-chain transaction ID")
    amount_nanoerg: int = Field(..., gt=0, description="Amount deposited in nanoERG")
    address: str = Field(..., min_length=1, description="Source address")
    notes: str = Field("", description="Optional notes")


class WithdrawalRequest(BaseModel):
    """Record a withdrawal from the house bankroll."""
    tx_id: str = Field(..., min_length=64, max_length=64, description="On-chain transaction ID")
    amount_nanoerg: int = Field(..., gt=0, description="Amount withdrawn in nanoERG")
    address: str = Field(..., min_length=1, description="Destination address")
    notes: str = Field("", description="Optional notes")
...
class LedgerBalanceResponse(BaseModel):
    total_deposits_nanoerg: int = 0
    total_deposits_erg: str = "0"
    total_withdrawals_nanoerg: int = 0
    total_withdrawals_erg: str = "0"
    total_bet_fees_nanoerg: int = 0
    total_bet_fees_erg: str = "0"
    total_bet_payouts_nanoerg: int = 0
    total_bet_payouts_erg: str = "0"
    total_bet_refunds_nanoerg: int = 0
    total_bet_refunds_erg: str = "0"
    net_balance_nanoerg: int = 0
    net_balance_erg: str = "0"
    deposit_count: int = 0
...
@router.post("/deposits", response_model=LedgerEntryResponse)
async def record_deposit(req: DepositRequest):
    """
    Record a deposit to the house bankroll.

    Called when ERG is sent to the house wallet address.
    The operator monitors incoming transactions and records them here
    for LP accounting and audit trail.
    """
    entry_id = record_ledger_entry(
        ...
        tx_id=req.tx_id,
        address=req.address,
        notes=req.notes or f"Deposit of {req.amount_nanoerg / 1e9:.9f} ERG from {req.address}",
    )

    # Verify tx exists on chain (optional, best-effort)
    try:
        tx_data = await _query_node(f"/blockchain/transaction/byId/{req.tx_id}")
        if not tx_data:
            raise HTTPException(
                status_code=404,
                detail=f"Transaction {req.tx_id} not found on-chain",
            )
        ...
        logger.warning("Could not verify deposit tx %s on-chain: %s", req.tx_id, e)
    ...
    return LedgerEntryResponse(
        ...
        type="deposit",
        amount_nanoerg=req.amount_nanoerg,
        amount_erg=f"{Decimal(req.amount_nanoerg) / Decimal('1e9'):.9f}",
        address=req.address,
        bet_id=None,
        timestamp=datetime.now(timezone.utc).isoformat(),
        notes=req.notes,
...
@router.post("/withdrawals", response_model=LedgerEntryResponse)
async def record_withdrawal(req: WithdrawalRequest):
    """
    Record a withdrawal from the house bankroll.

    Called when the operator sends ERG from the house wallet.
    Requires that the withdrawal does not exceed available capacity.
    """
    # Check that withdrawal doesn't exceed available capacity
    try:
        status = await bankroll_status()
        available = status.available_capacity_nanoerg
        if req.amount_nanoerg > available:
            raise HTTPException(
                status_code=400,
                detail=f"Withdrawal amount ({req.amount_nanoerg:,} nanoERG) exceeds "
                       f"available capacity ({available:,} nanoERG). "
                       f"Pending bets are locked."
            )
    except HTTPException:
        raise
    except Exception as e:
        logger.warning("Could not verify capacity for withdrawal: %s", e)
    ...
    entry_id = record_ledger_entry(
        ...
        tx_id=req.tx_id,
        address=req.address,
        notes=req.notes or f"Withdrawal of {req.amount_nanoerg / 1e9:.9f} ERG to {req.address}",
    )
...
async def get_ledger(
    ...
    bet_id: Optional[str] = Query(None, description="Filter by bet ID"),
    since: Optional[str] = Query(None, description="ISO 8601 start time"),
    until: Optional[str] = Query(None, description="ISO 8601 end time"),
    limit: int = Query(50, ge=1, le=500),
    offset: int = Query(0, ge=0),
    ...
    """
    Get paginated bankroll ledger entries.

    Returns all financial movements (deposits, withdrawals, bet fees,
    bet payouts, bet refunds) with optional filters.
    """
    entries, total = get_ledger_entries(
        entry_type=type,
        address=address,
        bet_id=bet_id,
        since=since,
    ...
    return LedgerListResponse(entries=entries, total=total)


@router.get("/ledger/balance", response_model=LedgerBalanceResponse)
async def get_ledger_balance_endpoint():
    """
    Get bankroll balance computed from ledger entries.

    Returns deposit/withdrawal totals and net balance for LP accounting.
    """
